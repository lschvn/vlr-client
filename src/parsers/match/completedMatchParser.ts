...
export class CompletedMatchParser {
  ...
  public parse(matchId: string): VlrCompletedMatch {
    ...
    this.$('.match-streams .match-streams-container > *').each((i, el) => {
      const $el = this.$(el);
      let name = '';
    ...
    this.$('.match-vods .wf-card a').each((i, el) => {
      const $el = this.$(el);
  ...
  private parseHeadToHead(): VlrHeadToHead[] {
    ...
    this.$('.match-h2h-matches a').each((_, el) => {
      ...
      const team1img =
        'https:' + this.$('.match-h2h-header-team').first().find('img').attr('src');
  ...
  private parseCompletedMatchMaps(): VlrCompletedMatchMap[] {
    ...
    this.$('.vm-stats-game[data-game-id]').each((i, el) => {
      ...
      const team1Score = parseInt(
        this.$(el).find('.team:not(.mod-right) .score').text().trim(),
        10,
      );
      const team2Score = parseInt(
        this.$(el).find('.team.mod-right .score').text().trim(),
        10,
      );

      const team1SideStatsSpans = this.$(el).find(
      ...
      const team2SideStatsSpans = this.$(el).find(
      ...
      tables
        ...
        .find('tbody tr')
        .each((_, playerRow) => {
          const stats: VlrPlayerStatsCompleted =
            this.parsePlayerStatsCompleted(playerRow);
    ...
    this.$(mapContainer)
      ...
      .each((i, roundEl) => {
        ...
        rounds.push({
          ...
          winningTeamSide,
          outcome,
          outcomeIconUrl: VLR_URL + outcomeIcon,
        });
      });
    return rounds;
  }
  ...
  private parsePlayerStatsCompleted(
    ...
    const getStat = (index: number): VlrPlayerStatValue => ({
      all: this.$(playerRow)
        ...
        .text()
        .trim(),
      attack: this.$(playerRow)
        .find('.mod-stat')
        .eq(index)
        .find('.side.mod-side.mod-t')
        .text()
        .trim(),
      defense: this.$(playerRow)
        .find('.mod-stat')
        .eq(index)
    ...
    return {
      ...
      agents: this.$(playerRow)
        .find('.mod-agents img')
        .map((_, agentEl) => ({
          name: this.$(agentEl).attr('title') || '',
          iconUrl: 'https:' + this.$(agentEl).attr('src'),
        }))
        .get(),
      rating: getStat(0),
      acs: getStat(1),
      ...
      d: {
        all: this.$(playerRow).find('.mod-vlr-deaths .side.mod-both').text().trim(),
        attack: this.$(playerRow).find('.mod-vlr-deaths .side.mod-t').text().trim(),
        defense: this.$(playerRow)
